...
public class DocumentCategoriesController : ControllerBase
{
    ...
    public DocumentCategoriesController(HRMSDbContext context) => _context = context;
    ...
    {
        var item = await _context.DocumentCategories.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
    ...
    public async Task<IActionResult> Create([FromBody] MasterDataUpsertDto dto, CancellationToken cancellationToken)
    {
        ...
        if (await _context.DocumentCategories.AnyAsync(x => x.Name == name, cancellationToken))
        {
            return Conflict("A document category with this name already exists.");
        }

        var entity = new DocumentCategory { Name = name, IsActive = dto.IsActive };
    ...
    public async Task<IActionResult> Update(int id, [FromBody] MasterDataUpsertDto dto, CancellationToken cancellationToken)
    {
        ...
        var entity = await _context.DocumentCategories.FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
        if (entity == null)
        {
            return NotFound();
        }
        ...
        var name = dto.Name.Trim();
        if (await _context.DocumentCategories.AnyAsync(x => x.Name == name && x.Id != id, cancellationToken))
        {
            return Conflict("A document category with this name already exists.");
        }
